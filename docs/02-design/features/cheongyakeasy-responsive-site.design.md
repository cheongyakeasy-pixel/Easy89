...
- Toss 스타일 준수: 인터랙션은 `#3182f6`, 본문은 한국어 중심, 숫자와 날짜는 정확하게 표시한다.
...
| 스타일링 | 일반 CSS + CSS 변수 | Toss 토큰을 전역 변수로 정의하고, 의존성을 줄인다. |
...
| `src/styles/*` | 없음 | Toss 토큰, 레이아웃, 반응형 규칙 |
...
| `Button` | `components/ui` | Toss 스타일 primary/weak 버튼 |
...
- 기본 폰트 스택은 Toss Product Sans 우선, 한국어 시스템 폰트 fallback을 사용한다.
...
- focus outline은 Toss Blue 계열로 보이게 유지한다.
...
| UI 방향 | Toss-inspired utility UI | 청약/금융 정보의 신뢰감과 가독성을 높인다. |
